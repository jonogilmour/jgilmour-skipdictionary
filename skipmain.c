...
int main(int argc, const char * argv[])
{
	...
	while(fgets(buffer, LINE_MAX, kvpairs) != NULL) {
		...
		comparisons += addSkipEntry(dict, key, value, added, compareFunction);
		if(!*added) duplicates++;
		else inserted += *added;
		free(value);
	...
	while(fgets(buffer, LINE_MAX, stdin) != NULL && strcmp(buffer, "!exit\n")) {
		...
		searchresult = skipDictSearch(dict, querykey, cmpr, compareFunction);
	...
	purgeSkipDict(dict, compareFunction);
	free(dict);
	dict = NULL;
	free(added);
	added = NULL;
