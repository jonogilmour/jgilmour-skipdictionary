...
/*
 Prints the entire contents of a skip list dictionary by traversing the lowest level
*/
void printSkipDict(skipDict* theSkipDict)
{
	skipDictEntry* curNode = theSkipDict->head->next[0];
	printf("Printing skipdict with maximum level: %d\n", theSkipDict->level);
	if (curNode == NULL) {
		printf("Empty dictionary\n");
	}
	
	else while(curNode != NULL) {
...
int addSkipEntry(skipDict* theSkipDict, char* key, char* value, int* added, int (*keycmp)(char*, char*))
{
	...
	*added = 0;
	...
	if(curNode == NULL || keycmp(curNode->key, key)) { 
		...
		*added = 1;
		curNode = newSkipEntry(key, value, newlevel);
...
void purgeSkipDict(skipDict* theSkipDict, int (*keycmp)(char*, char*)) {
	...
	while(curNode != NULL) {
		...
		skipDictDelete(theSkipDict, curNode->key, keycmp);
		curNode = nextNode;
	}
}
